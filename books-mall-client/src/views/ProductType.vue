<template>
  <div class="product-type-box">
    <a-spin :spinning="loading.all">
      <p class="product-type-info">
        {{ typeName }}分类书籍 共计：{{ data.length }}
      </p>
      <a-empty v-if="showEmpty">
        <span slot="description">该分类下没有书籍</span>
        <a-button type="primary" @click="handleReturnPageBtnClick"
          >返回</a-button
        >
      </a-empty>
      <div v-if="data.length !== 0">
        <div style="height: 40px;padding-left: 12px">
          <a-radio-group
            defaultValue="gmtModified"
            buttonStyle="solid"
            @change="handleSortRadioChange"
          >
...
export default {
  ...
  methods: {
    ...
    initAllCommodityByType() {
      ...
      Get(
        `${API.commodity.type}?typeId=${this.id}&page=${this.pagination.page -
          1}&size=${this.pagination.size}&sort=${this.pagination.sort}`
      )
        .withSuccessCode(200)
        .withErrorStartMsg("获取分类书籍失败：")
        .do(response => {
